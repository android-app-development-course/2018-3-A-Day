...
import com.example.bottombar.step.accelerometer.StepCount;
...
public class StepService extends Service implements SensorEventListener {
     ...
     * 保存折线图x轴上各整点步数
     */
    private int timeNum=7;
    public static int TIME_STEP[]=new int[7];
    /**
...
*/
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            for(int i=0;i<timeNum;i++)
            {
                if(list.get(0).getTimeStep(i)!=null) TIME_STEP[i]=Integer.parseInt(list.get(0).getTimeStep(i));
            }
