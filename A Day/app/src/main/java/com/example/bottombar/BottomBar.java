...
public class BottomBar extends View {
    ...
    protected void switchFragment(int whichFragment) {
        ...
        if(whichFragment==0 && main_flag==1) {
            ...
            transaction.replace(frameLayoutId,noteFragment);
            transaction.commit();
            main_flag=1;
        }
    }

    public void setCurrentStep(int currentStepCount)
    {
        this.currentStepCount=currentStepCount;
        if(main_flag==0) show_walk_main();
    }

    public void show_walk_main()
    {
        walkFragment=new WalkFragment();
        Bundle bundle = new Bundle();
        bundle.putInt("stepCount",currentStepCount);
        walkFragment.setArguments(bundle);//数据传递到fragment中

        FragmentTransaction transaction = ((AppCompatActivity) context).getSupportFragmentManager().beginTransaction();
        transaction.replace(containerId,walkFragment);
        transaction.commit();
    }
}
